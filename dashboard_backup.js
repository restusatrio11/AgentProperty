...
export default function AgentDashboard() {
  ...
  const [adminFormData, setAdminFormData] = useState({ id: "", email: "", password: "", role: "ADMIN", isActive: true });
  ...
  const renderAuditSortIcon = (field) => {
    if (auditSortField !== field) return <span className={styles.sortIconInactive}>⇅</span>;
    return auditSortDirection === "asc" ? <span className={styles.sortIconActive}>▲</span> : <span className={styles.sortIconActive}>▼</span>;
  };
  ...
  const renderSessionSortIcon = (field) => {
    if (sessionSortField !== field) return <span className={styles.sortIconInactive}>⇅</span>;
    return sessionSortDirection === "asc" ? <span className={styles.sortIconActive}>▲</span> : <span className={styles.sortIconActive}>▼</span>;
  };
  ...
  const renderAdminSortIcon = (field) => {
    if (adminSortField !== field) return <span className={styles.sortIconInactive}>⇅</span>;
    return adminSortDirection === "asc" ? <span className={styles.sortIconActive}>▲</span> : <span className={styles.sortIconActive}>▼</span>;
  };
  ...
  const renderRbacSortIcon = () => {
    return rbacSortDirection === "asc" ? <span className={styles.sortIconActive}>▲</span> : <span className={styles.sortIconActive}>▼</span>;
  };
  ...
  const handleCreateAdminSubmit = async (e) => {
    ...
    try {
      ...
      const payload = adminFormMode === "create" 
        ? { email: adminFormData.email, password: adminFormData.password }
        : { email: adminFormData.email, role: adminFormData.role, isActive: adminFormData.isActive };
  ...
  const handleRoleChange = (newRole) => {
    setRole(newRole);
    document.cookie = `user_role=${newRole}; path=/; max-age=2592000; SameSite=Lax`;
    addToast(`Peran disimulasikan: ${newRole === "SUPERADMIN" ? "Superadmin (CRUD Aktif)" : "Admin (Read-only)"}`);
    
    // Check if the current tab is allowed for the new role, if not fallback to "dashboard"
    const allowed = MENU_ITEMS.find(item => item.id === activeTab && hasPermission(newRole, item.resource, item.action, permissionsMatrix));
    if (!allowed) {
        ...
        <div className={styles.sidebarFooter}>
          {!sidebarCollapsed ? (
            <>
              ...
              <div className={styles.roleSimulator}>
                <label className={styles.simulatorLabel}>SIMULASI PERAN:</label>
                <select
                      ...
                      <tr key={p.id} className={styles.customTableRow}>
                        ...
                        <td style={{ textAlign: "center" }}>
                          <button
                            ...
                            style={{ padding: "6px 12px", fontSize: "11px", backgroundColor: "#1b8a5a" }}
                            onClick={() => handleRestore(p.id, p.namaProperti)}
                          >
                            <History size={14} style={{ marginRight: "4px" }} /> Pulihkan
                          </button>
                        </td>
                      </tr>
                    ))}
                      ...
                      <thead>
                        <tr>
                          ...
                          <th onClick={() => handleAuditSort("changeSummary")} className={styles.sortHeader}>
                            Ringkasan Perubahan {renderAuditSortIcon("changeSummary")}
                          </th>
                                ...
                                <div style={{ display: "flex", gap: "8px", justifyContent: "center" }}>
                                  <button 
                                    ...
                                    style={{ 
                                      ...
                                      padding: "0 12px", 
                                      fontSize: "11px",
                                      backgroundColor: t.isApproved ? "#f3f4f6" : "#c9a961",
                                      color: t.isApproved ? "#374151" : "#fff",
                                      border: t.isApproved ? "1px solid #d1d5db" : "none"
                                    }}
                                    onClick={() => handleToggleTestimonial(t.id, t.isApproved)}
                                  >
                                    {t.isApproved ? "Sembunyikan" : "Tampilkan"}
                                  </button>
                                  <button 
                                    className={`${styles.tableActionBtn} ${styles.tableActionBtnDelete}`}
                        ...
                        {paginatedActiveSessions.map((session, idx) => {
                          ...
                          return (
                            <tr key={session.id} className={styles.customTableRow}>
                              ...
                              <td className={styles.actionCell}>
                                <button
                                  className={styles.tableActionBtnDelete}
                                  style={{ border: "none", padding: "6px 12px", borderRadius: "6px", fontSize: "11px" }}
                  ...
                  <tbody>
                    {paginatedAdmins.map((admin) => (
                      <tr key={admin.id} className={styles.customTableRow}>
                        <td style={{ fontWeight: 600, color: "#1a1a1a" }}>{admin.email}</td>
                        <td>
                        ...
                        <td style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "nowrap" }}>
                          <button
            ...
            <form onSubmit={handleCreateAdminSubmit}>
              ...
              <div className={styles.modalFooter}>
                ...
                <button type="submit" className={styles.btnSubmit}>
                  {adminFormMode === "create" ? "Buat Akun" : "Simpan Perubahan"}
                </button>
              </div>
            </form>
          </div>
      ...
      {isDeleteAdminOpen && adminToDelete && (
        <div className={styles.modalOverlay}>
          <div className={`${styles.modal} ${styles.deleteModal}`}>
            ...
            <div className={styles.modalFooter}>
              <button type="button" className={styles.btnCancel} onClick={() => setIsDeleteAdminOpen(false)}>Batal</button>
              <button type="button" className={styles.btnDeleteConfirm} onClick={handleDeleteAdminConfirm}>Ya, Hapus Admin</button>
            </div>
          </div>
        </div>
      )}
